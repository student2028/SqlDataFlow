...
public class Replace extends BaseProcessor {
    ...
    public void process(SparkSession spark) {
        ...
        for (Config c : fields) {
            df = df.withColumn(
                    c.getString("target_field"),
                    regexp_replace(col(c.getString("source_field")), c.getString("pattern"), c.getString("replacement"))
            );
        }
